...
export default async function OrderPage({
  ...
  const [{ order, transactions, partner }, mystery] = await Promise.all([
    getOrder(id),
    getOrderMysteryDetails(id),
  ]);
  if (!order) notFound();
  return (
    <main className="page-shell max-w-3xl py-7 sm:py-10">
      <Link
        href="/orders"
      ...
      <Card className="overflow-hidden p-0">
        ...
        <div className="space-y-4 p-6 sm:p-8">
          <div className="grid gap-4 text-sm sm:grid-cols-2">
            <div>
              <p className="text-xs text-muted">申请时间</p>
            ...
            {order.approved_at && (
              <div>
                <p className="text-xs text-muted">确认时间</p>
                <p className="mt-1 font-semibold text-brown">
                  {formatDate(order.approved_at, true)}
                </p>
              </div>
            )}
            {order.completed_at && (
          ...
          {order.mystery_status && order.mystery_status !== "revealed" && (
            <div className="rounded-3xl bg-brown p-5 text-center text-white">
              ...
              <p className="mt-2 text-sm text-white/65">
                {order.mystery_status === "ready"
                  ? "点一下，由你亲手揭晓。"
                  : "真实内容仍然保密，准备好后这里会出现揭晓按钮。"}
              </p>
              {order.mystery_status === "ready" && (
                <form action={revealMysteryOrderAction} className="mt-4">
                  <input type="hidden" name="order_id" value={order.id} />
                  <SubmitButton pendingText="正在揭晓…">
          ...
          {order.mystery_status === "revealed" && mystery && (
            <div className="success-pop rounded-3xl border border-rose-200 bg-gradient-to-br from-rose-50 to-amber-50 p-6 text-center">
              ...
              <h2 className="mt-2 text-2xl font-black text-brown">
                {mystery.surprise_title}
              </h2>
              <p className="mt-3 leading-7 text-muted">
                {mystery.surprise_content}
              </p>
              {mystery.admin_message && (
                <p className="mt-4 rounded-2xl bg-white/80 p-3 text-sm font-semibold text-brown">
                  “{mystery.admin_message}”
                </p>
              )}
            </div>
          ...
          {order.status === "pending" && (
            <form action={cancelOrderAction} className="pt-2">
              <input type="hidden" name="order_id" value={order.id} />
              <SubmitButton
      ...
      <div className="mt-7">
        <h2 className="mb-3 text-lg font-black text-brown">奶龙币记录</h2>
        <Card className="divide-y divide-line p-2">
          {transactions.map((item) => (
            <div key={item.id} className="flex items-center gap-3 p-3">
              <span className="flex size-9 items-center justify-center rounded-full bg-amber-100 text-nailong-deep">
