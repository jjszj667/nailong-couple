  ...
  return (
    <main className="page-shell py-7 sm:py-10">
      <div className="mb-7">
        ...
        <h1 className="mt-1 text-3xl font-black text-brown">奶龙币钱包</h1>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          <p className="text-xs text-muted">钱包状态</p>
          <p className="mt-2 flex items-center gap-2 font-bold text-green-700">
      ...
      <section className="mt-8">
        <h2 className="mb-3 text-xl font-black text-brown">全部流水</h2>
        {transactions.length ? (
          <Card className="divide-y divide-line p-2">
            {transactions.map((item) => {
              const income =
                item.direction === "income" || item.direction === "unfreeze";
              const fromPartner =
                item.operator_id && item.operator_id === partner?.id;
              const title =
                fromPartner && item.direction === "income"
                  ? `${partner.nickname} 偷偷塞给你 ${item.amount} 奶龙币`
                  : item.reason;
              return (
                <div
                  key={item.id}
                ...
                >
                  <span
                    className={`flex size-10 shrink-0 items-center justify-center rounded-full ${income ? "bg-green-100 text-green-700" : "bg-amber-100 text-orange"}`}
                  >
                    {income ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-muted">
                      {fromPartner && item.reason !== title
                        ? `${item.reason} · `
                        : ""}
                      {formatDate(item.created_at, true)} · 可用{" "}
                      {item.after_balance} / 冻结 {item.after_frozen_balance}
                    </p>
                  </div>
                  <span
                    className={`font-bold tabular-nums ${item.amount > 0 ? "text-green-700" : "text-brown"}`}
                  >
                    {item.amount > 0 ? "+" : ""}
                    {item.amount}
                  </span>
