import type { Product, ProductMysteryDetails } from "@/types/database";
    ...
    <form action={saveProductAction} className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          <label className="block text-sm font-semibold text-brown">商品类型<select name="product_type" className="field mt-2" defaultValue={product?.product_type ?? "normal"}><option value="normal">普通商品</option><option value="mystery">惊喜箱</option></select></label>
          <label className="mt-3 block text-sm font-semibold text-brown">公开提示<input name="mystery_hint" className="field mt-2" defaultValue={product?.mystery_hint ?? "里面是什么？兑换以后才知道。"} maxLength={300} /></label>
          <p className="mt-3 text-xs leading-5 text-muted">普通商品可以忽略下面两项；惊喜箱必须填写，真实内容只保存在管理员可读的秘密表。</p>
          <label className="mt-3 block text-sm font-semibold text-brown">真实惊喜名称<input name="surprise_title" className="field mt-2" defaultValue={mystery?.surprise_title ?? ""} maxLength={100} /></label>
          <label className="mt-3 block text-sm font-semibold text-brown">真实惊喜内容<textarea name="surprise_content" className="field mt-2 min-h-24" defaultValue={mystery?.surprise_content ?? ""} maxLength={1000} /></label>
        </div>
